...
class FacadeUpdater2(chainer.training.StandardUpdater):

    def __init__(self, *args, **kwargs):
        self.enc0, self.dec0, self.enc, self.dec, self.dis = kwargs.pop('models')
        super().__init__(*args, **kwargs)


    def loss_enc(self, enc, y_out):
        batchsize,_,w,h = y_out.data.shape
        loss_adv = F.sum(F.softplus(-y_out)) / batchsize / w / h
        loss = loss_adv
        chainer.report({'loss': loss}, enc)
        return loss
        
    def loss_dec(self, dec, y_out):
        batchsize,_,w,h = y_out.data.shape
        loss_adv = F.sum(F.softplus(-y_out)) / batchsize / w / h
        loss = loss_adv
        chainer.report({'loss': loss}, dec)
    ...
    def update_core(self):        
        ...
        enc0, dec0, enc, dec, dis = self.enc0, self.dec0, self.enc, self.dec, self.dis
        xp = enc.xp
        ...
        with chainer.using_config('train', False), chainer.using_config('enable_backprop', False):
            x_in = Variable(dec0(enc0(x_in)).data)
        ...
        enc_optimizer.update(self.loss_enc, enc, y_fake)
        for z_ in z:
            z_.unchain_backward()
        dec_optimizer.update(self.loss_dec, dec, y_fake)
        x_in.unchain_backward()
        x_out.unchain_backward()
        dis_optimizer.update(self.loss_dis, dis, y_real, y_fake)
