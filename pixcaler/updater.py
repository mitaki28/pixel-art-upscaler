...
class LsGanLoss:
    def dis_fake(self, y_fake):
        return 0.5 * F.mean(y_fake ** 2)

    def dis_real(self, y_real):
...
class Pix2PixUpdater(chainer.training.StandardUpdater):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.adv_loss = kwargs.pop('adv_loss')
        super().__init__(*args, **kwargs)

    def loss_func_rec_gen(self, x_in, x_out):
        return F.mean_absolute_error(x_out, x_in)

    def loss_gen(self, enc, x_out, x_in, y_fake):
        loss_rec = self.loss_func_rec_gen(x_in, x_out)
        loss_adv = self.adv_loss.gen(y_fake)
        loss = self.lam1 * loss_rec + self.lam2 * loss_adv
        chainer.report({'loss_rec': loss_rec}, enc)
        chainer.report({'loss_adv': loss_adv}, enc)
        return loss

    def loss_dis(self, dis, y_real, y_fake):
        L1 = self.adv_loss.dis_real(y_real)
        L2 = self.adv_loss.dis_fake(y_fake)
        loss = L1 + L2
