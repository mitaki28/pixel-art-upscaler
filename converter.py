...
def export(model_dir, iteration, w_in=64, in_ch=4, out_ch=4, output_dir=Path(__file__).parent/'frontend'/'public'/'model'/'webdnn'):
    ...
    x_in = chainer.Variable(np.zeros((1, in_ch, w_in, w_in), dtype=np.float32))
    x_out = dec(enc(x_in))
